    ...
    async login (req, res) {
        ...
        try {
            ...
            const [rolDatos] = await db.query(
                'SELECT r.nombre AS rol p.nombre AS permiso FROM roles r JOIN rol_permiso rp ON r.is_rol = rp.id_rol JOIN permisos p ON rp.permiso_id = p.id_permiso WHERE r.id_rol = ?',
                [usuario.id_rol]
            );
